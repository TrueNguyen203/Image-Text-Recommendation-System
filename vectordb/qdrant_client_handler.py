...
class QdrantHandler:
    ...
    def search(self, query_vector: List[float], limit: int = 12, filter_criteria: Optional[Dict] = None):
        """
        Tìm kiếm vector với bộ lọc nhiều điều kiện.
        filter_criteria: Dict {'brand': 'Nike', 'color': 'Black'}
        """
        ...
        if filter_criteria:
            ...
            for key, value in filter_criteria.items():
                if value and value.strip() != "":
                    # Sử dụng MatchValue cho string chính xác
        ...
        # 2. Thực hiện Search (Hỗ trợ fallback version)
        try:
